...
import com.moyo.backend.follow.exception.FollowErrorCode;
...
class GithubFollowControllerTest {
    ...
    void 맞팔탐지기_에러코드_문서화(FollowErrorCode errorCode) throws Exception {
        // given
        doThrow(new MoyoException(errorCode)).when(githubFollowService).detectFollowUserList(anyLong(), any());
    ...
    void 깃허브_팔로우_캐시_삭제_성공() throws Exception {
        // given
        willDoNothing().given(githubFollowService).clearFollowCache(anyLong());
