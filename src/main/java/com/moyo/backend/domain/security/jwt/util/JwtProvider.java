package com.moyo.backend.domain.security.jwt.util;
...
public class JwtProvider {
	...
	public String createJwtToken(GithubOAuth2User user, JWK jwk, String type) throws JOSEException {
		...
		if (type.equals(JWT_REFRESH_TYPE))
			expirationTime = new Date(new Date().getTime() + JWT_REFRESH_TOKEN_EXPIRATION_MINUTE);
		else if (type.equals(JWT_ACCESS_TYPE))
			expirationTime = new Date(new Date().getTime() + JWT_ACCESS_TOKEN_EXPIRATION_MINUTE);
		else
