...
public class GithubFollowRelationReader {
	...
	public GithubFollowRelation findByUserId(Long userId, boolean forceSync) {
		...
		log.info("{}의 팔로워, 팔로잉 페이지 정보 로그 (page size = {}) | followerMaxPage : {} , followingMaxPage : {},", userId,
			GITHUB_MAX_QUERY_PAGING_SIZE, followStats.maxFollowerPageSize(), followStats.maxFollowingPageSize());
		...
		for (int currentPage = 1; currentPage <= maxFollowerPageSize; currentPage++) {
			...
			List<GithubFollowUserResponse> followersResponseList =
				githubFollowClient.fetchPagedFollowers(GITHUB_MAX_QUERY_PAGING_SIZE, currentPage, githubAccessToken);
		...
		for (int currentPage = 1; currentPage <= maxFollowingPageSize; currentPage++) {
			...
			List<GithubFollowUserResponse> followingsResponseList =
				githubFollowClient.fetchPagedFollowings(GITHUB_MAX_QUERY_PAGING_SIZE, currentPage, githubAccessToken);
